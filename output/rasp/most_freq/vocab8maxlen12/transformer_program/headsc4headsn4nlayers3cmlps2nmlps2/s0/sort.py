...
def run(tokens):
    ...
    classifier_weights = pd.read_csv(
        "output/rasp/most_freq/vocab8maxlen12/transformer_program/headsc4headsn4nlayers3cmlps2nmlps2/s0/sort_weights.csv",
        index_col=[0, 1],
        dtype={"feature": str},
    )
    # inputs #####################################################
    ...
    def predicate_0_0(position, token):
        if position in {0, 9, 3, 11}:
            return token == "1"
        elif position in {1, 4}:
            return token == "0"
        elif position in {2}:
            return token == "<s>"
        elif position in {8, 10, 5, 7}:
            return token == "4"
        elif position in {6}:
            return token == "3"
    ...
    def predicate_0_3(position, token):
        if position in {0, 1, 9}:
            ...
            return token == "2"
        elif position in {3, 4, 6, 7, 8, 10, 11}:
            return token == "3"
        elif position in {5}:
            return token == "4"
    ...
    def num_predicate_0_3(position, token):
        if position in {0}:
            return token == "1"
        elif position in {1, 7, 8, 9, 10, 11}:
            return token == "0"
        elif position in {2}:
            return token == ""
    ...
    # attn_1_1 ####################################################
    def predicate_1_1(q_mlp_0_0_output, k_mlp_0_0_output):
        if q_mlp_0_0_output in {0, 11, 4, 7}:
            return k_mlp_0_0_output == 2
        elif q_mlp_0_0_output in {1}:
            return k_mlp_0_0_output == 1
        elif q_mlp_0_0_output in {2, 3}:
            return k_mlp_0_0_output == 7
        elif q_mlp_0_0_output in {5, 6}:
            return k_mlp_0_0_output == 3
        elif q_mlp_0_0_output in {8, 9, 10}:
            return k_mlp_0_0_output == 5

    attn_1_1_pattern = select_closest(mlp_0_0_outputs, mlp_0_0_outputs, predicate_1_1)
    ...
    def predicate_1_2(mlp_0_0_output, token):
        if mlp_0_0_output in {0, 1}:
            ...
            return token == "<s>"
        elif mlp_0_0_output in {4, 5, 6, 7, 9, 10}:
            return token == "4"
    ...
    def num_predicate_1_2(position, token):
        if position in {0}:
            return token == "2"
        elif position in {1, 2, 4, 5, 6, 7, 8}:
            return token == "<s>"
        elif position in {3}:
            return token == "1"
    ...
    # num_mlp_1_1 #################################################
    def num_mlp_1_1(num_attn_0_0_output, num_attn_0_2_output):
    ...
    # num_attn_2_0 ####################################################
    def num_predicate_2_0(attn_1_0_output, attn_1_3_output):
        if attn_1_0_output in {"4", "1", "</s>", "<s>", "3", "0"}:
            return attn_1_3_output == "1"
        elif attn_1_0_output in {"2"}:
            return attn_1_3_output == "2"

    num_attn_2_0_pattern = select(attn_1_3_outputs, attn_1_0_outputs, num_predicate_2_0)
    ...
    # num_attn_2_1 ####################################################
    def num_predicate_2_1(mlp_1_1_output, attn_0_2_output):
        if mlp_1_1_output in {0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11}:
            return attn_0_2_output == "3"
        elif mlp_1_1_output in {3}:
            return attn_0_2_output == "2"

    num_attn_2_1_pattern = select(attn_0_2_outputs, mlp_1_1_outputs, num_predicate_2_1)
    ...
    # mlp_2_1 #####################################################
    def mlp_2_1(mlp_0_0_output, num_mlp_0_0_output):
    ...
    mlp_2_1_outputs = [
        mlp_2_1(k0, k1) for k0, k1 in zip(mlp_0_0_outputs, num_mlp_0_0_outputs)
    ]
    ...
    # num_mlp_2_0 #################################################
    def num_mlp_2_0(num_attn_0_2_output, num_attn_2_2_output):
        key = (num_attn_0_2_output, num_attn_2_2_output)
        if key in {
            (2, 1),
            (3, 1),
            (4, 0),
            ...
            return 7
        elif key in {(0, 0), (1, 0), (2, 0), (3, 0)}:
            return 11
        return 8

    num_mlp_2_0_outputs = [
        num_mlp_2_0(k0, k1)
        for k0, k1 in zip(num_attn_0_2_outputs, num_attn_2_2_outputs)
    ]
